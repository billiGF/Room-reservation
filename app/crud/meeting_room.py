...
async def get_all_meetingroom(session: AsyncSession):
...
async def get_meeting_room_by_id(room_id: int, session: AsyncSession):
...
async def update_meeting_room(db_room: MeetingRoom, room_id: MeetingRoomCreate, session: AsyncSession):
